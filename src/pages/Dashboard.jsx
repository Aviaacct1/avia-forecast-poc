...
const LineChart = ({ data, title, xLabel = 'Year', yLabel = 'Value' }) => {
          ...
          return (
            <div key={idx} className="flex-1 flex flex-col items-center">
              <div
                ...
                title={`${item.label}: ${item.value.toLocaleString('en-US', { maximumFractionDigits: 1 })}`}
              />
  ...
  const getRegionalData = useMemo(() => {
    ...
    countryAirports.forEach((airport) => {
      ...
      if (!countryMap[country]) {
        countryMap[country] = { pax2024: 0, pax2030: 0, pax2044: 0, pax: 0, rpk2024: 0, rpk2030: 0, rpk2044: 0, rpk: 0, airports: 0 };
      }
  ...
  if (view === 'regional' && getRegionalData) {
    return (
      <div className="min-h-screen bg-slate-50">
        ...
        <div className="max-w-7xl mx-auto px-6 py-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
            <LineChart data={getRegionalData.map((c) => ({ label: c.name.substring(0, 8), value: c.pax / 1e9 }))} title={`${selectedRegion} - Passengers by Country`} />
            <LineChart data={getRegionalData.map((c) => ({ label: c.name.substring(0, 8), value: c.rpk }))} title={`${selectedRegion} - RPK by Country`} />
          </div>
      ...
      <div className="min-h-screen bg-slate-50">
        ...
        <div className="max-w-7xl mx-auto px-6 py-8">
          <div className="bg-white rounded-lg shadow border border-slate-200 p-6 mb-8">
            <h3 className="text-lg font-bold mb-6">Forecast Summary (2024, 2030, 2044)</h3>
